<script>
  ...
  $(document).ready(function() {
    tabel = $('#myTable').DataTable({
      ...
      "ajax":
      {
          "url": "<?php echo base_url('category/view') ?>", // URL file untuk proses select datanya
          "type": "POST"
      },
      "deferRender": true,
      "columns": [
        { "data": "category_name" }, // Tampilkan nama
        { "data": "description" },  // Tampilkan username
        { "render": function ( data, type, row ) {  // Tampilkan status
          var status = ""
          if(row.status == 1){ // Jika status 1
          	status = 'Aktif' // Set Aktif
          } else { // Jika bukan 1
            status = 'Tidak Aktif' // Set Status
          }
            return status; // Tampilkan jenis status
          }
        },
        { sWidth: "17%", 
    ...
    $('#myTable').on('click', 'tbody .edit_btn', function () {
      ...
      $.redirect('<?php echo base_url().$this->router->class.'/update'; ?>', {'id': data_row['id_category']}, 'GET');
    });
  });
</script>